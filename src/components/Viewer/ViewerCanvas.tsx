  ...
  forwardRef<ViewerCanvasHandle, ViewerCanvasProps>(
    ...
    return (
      <div className="relative flex-1 min-h-0 w-full">
        ...
        <div className="pointer-events-none absolute inset-x-4 top-4 flex items-center justify-between rounded-xl bg-slate-900/60 px-3 py-2 text-xs text-slate-100 shadow-lg backdrop-blur">
          <span>Three.js 视窗</span>
          <span>
            {model?.name || "示例模型"} •{" "}
            {model?.url ? "外部资源" : "本地占位"}
          </span>
